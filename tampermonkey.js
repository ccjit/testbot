...
(function() {
    ...
    let ownerresponses = [
        ...
        "You know what? If the command name is \"owner\", I might aswell make it do something useful. Bot owner: ccjt & Test Account"
    ]
    ...
    let categoryresponses = [
        ...
        "Uncaught ReferenceError: That is not a valid category. Categories: ",
    ...
    let oldhelp = false
    // startup message
    MPP.chat.send("TestBot is on. Use 〈 !help 〉 to see commands.")
    // main script
    MPP.client.on("a", function (m) {
        ...
        let tid = m.p.id
        let id = m.p._id
        let crowned = function() { if ('crown' in MPP.client.channel) { if (MPP.client.channel.crown.userId == MPP.client.getOwnParticipant()._id) { return true; } else { return false; } } else { return false; } }
        let whitelisted = function(id){if(whitelist.includes(id)){return true}else{return false}}
        ...
        let addToWhitelist = function(id){whitelist.push(id);localStorage.setItem("whitelist", whitelist);}
        // commands
        if (c == "!fun") {
        ...
        if (c == "!about") {
            send("TestBot - Made by ccjt & Test Account | Programming: ccjt - Ideas: Test Account | This bot is open-source on GitHub and free for you to use. | Made in 2025 | Running version " + version)
      ...
      if (c == "!help") {
          if (!oldhelp) {
              if (a.length == 1) {
                  if (whitelisted(id)) {
                      ...
                      if (whitelisted(id) && a[1] != "info" && a[1] != "fun" && a[1] != "personal" && a[1] != "owner") {
                          send(categoryresponses[Math.floor(Math.random()*categoryresponses.length)] + "info, fun, personal, owner")
                      } else if (!whitelisted(id) && a[1] != "info" && a[1] != "fun"){
        ...
        if (whitelisted(id)) {
            ...
            if (c == "!crown") {
                if (crowned() || (crowned() && whitelisted(id))) {
                    MPP.client.sendArray([{
                        "m": "chown",
